...
if (isset($_POST['signup'])) {
    ...
    $input_specialist = sanitizeInput($_POST['Specialist']);
    $input_fee = sanitizeInput($_POST['fee']);
    ...
    if (empty($input_fee)) {
        $fee_err = "Please Enter Profesional Fee";
    } else {
        $fee = $input_fee;
    }
    ...
    if (empty($name_err) && empty($address_err) && empty($email_err) && empty($mobile_err) && empty($Specialist_err) && empty($gender_err) && empty($password_err) && empty($fee_err)) {
        $q = "INSERT INTO doctor (doctor_name,doctor_specialist,doctor_cost,doctor_mobile,doctor_gender,doctor_email,doctor_password,doctor_address) 
              VALUES ('{$name}','{$Specialist}','{$fee}','{$mobile}','{$gender}','{$email}','{$password}','{$address}')";
...
<html>
	...
	<body>
		...
		<div class="bg-img">
			<div style="margin-top: 10px; margin-left: 495px; background: #C0C0C0; width: 400px; height: 840px; opacity: 0.7; border-radius: 50px;">
				<h3 style="background-color: #C0C0C0; padding-left: 130px; padding-top: 30px;padding-bottom: 30px; border-radius: 50px;">Add Doctor</h3>
				<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" style="padding-left: 50px">
					...
					<input type="tel" name="mobile" placeholder="xxxxxxxxxxx"><br><?php echo $mobile_err ?><br>
					...
					<textarea rows="2",cols="40" style="margin-left: 0px" name="address"></textarea><br><?php echo $address_err ?><br>
		 			<button type="submit" value="Submit" name="signup" style="margin-left: 90px;">ADD</button>
				</form>
			</div>
		</div>
	</body>
