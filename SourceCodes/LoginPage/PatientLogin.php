...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($row['patient_email'] == $email && $row['patient_password'] == $hashedPassword) {
                $bool = true;
                break;
            }
        }
...
<body>
    <div style="width: 300px; height: 380px; border-radius: 50px; background-color: #C0C0C0; margin-left: 40%; margin-top: 100px;"><br>
        <h3 style=" padding-left: 80px; margin-top: 40px; border-radius: 20px 20px 0px 0px; ">Patient Login</h3><br>
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" style="padding-left: 30px;">
            ...
            <a href="../SignUp/Patient.php" style="padding-left: 55px;"><label>Create An Account</label></a>
        </form>
    </div>
</body>
</html>
